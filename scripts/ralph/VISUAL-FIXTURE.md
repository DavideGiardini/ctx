...
| `task-10-search-row-inside-the-assistant-turn` | `search-turn` | a model-invoked search row sitting **flush** inside the assistant's turn (no blank line splitting it off) with its own violet left bar and a glyph in the meta slot. Not a shipped bug — the task-10 acceptance itself, kept here so the pair cannot rot. The glyph is `⌕`, which cairosvg's fallback font renders as tofu: `tests/test_message_row.py` pins the character, the eye only confirms the slot is occupied |
