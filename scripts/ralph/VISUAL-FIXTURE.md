...
| `task-10-search-row-inside-the-assistant-turn` | `search-turn` | a model-invoked search row sitting **flush** inside the assistant's turn (no blank line splitting it off), wearing the context-green left bar of injected material rather than an unstyled row's system grey. Not a shipped bug — the task-10 acceptance itself, kept here so the pair cannot rot |
