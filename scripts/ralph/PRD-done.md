...
- [x] **37. UI: rebuild the diff view as a full-screen two-pane node diff** _(deps:
...
- [x] **40. UI: blank-line separation before a compression node** _(deps: none;
...
- [x] **41. UI: range selection uses hover styling, bridged across gaps** _(deps:
...
- [x] **45. Verify the polished compression UI end-to-end (qa-tester)** _(deps:
...
- [x] 47 — **Core+config: reframe `draft_compression` + rewrite the default prompt**
...
- [x] 48 — **UI: drop the zero-token interrupted node on cancel** — when a stream is
